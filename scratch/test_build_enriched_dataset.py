...
def validate_review_rows(rows, expected_nct_ids):
    ...
    if len(row_nct_ids) != 351:
        raise ValueError(f"Expected 351 rows, got {len(row_nct_ids)}")
...
for field in rows[0].keys():
    non_empty = sum(1 for r in rows if r[field].strip() != "")
    print(f"  {field}: {non_empty} non-empty / {len(rows)}")
